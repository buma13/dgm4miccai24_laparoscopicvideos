...
# script taken and adapted from https://huggingface.co/docs/diffusers/en/training/text2image
...
DATASET_NAME_MAPPING = {
    "<huggingface hub dataset name>": ("file_name", "text"),
}
...
def log_validation(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, epoch):
    ...
    for i in range(len(args.validation_prompts)):
        with torch.autocast("cuda"):
            image = pipeline(
                args.validation_prompts[i], num_inference_steps=200, generator=generator, safety_checker=None,
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        required=False,
        help="Run name to be displayed in wandb",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--random_flip",
        action="store_true",
        help="whether to randomly flip images horizontally and vertically",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--random_rotate", action="store_true", default=False, help="Enable random rotation augment during training."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to unfreeze the text encoder during training",
    )
...
def main():
    ...
    if is_wandb_available():
        if args.run_name:
            wandb.init(project="laparodiffusion", name=args.run_name)
        else:
            wandb.init(project="laparodiffusion")
    ...
    params_to_optimize = (
        itertools.chain(unet.parameters(), text_encoder.parameters()) if args.unfreeze_text_encoder else unet.parameters()
    )
    ...
    if accelerator.is_main_process:
        ...
        if args.validation_prompts is not None:
            ...
            for i in range(len(args.validation_prompts)):
                with torch.autocast("cuda"):
                    image = pipeline(
                        args.validation_prompts[i], num_inference_steps=100, generator=generator, safety_checker=None, guidance_scale=3,
                        height=args.resolution, width=args.resolution).images[0]
                images.append(image)
