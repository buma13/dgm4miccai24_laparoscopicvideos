...
# script taken and adapted from https://huggingface.co/docs/diffusers/en/training/controlnet
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable random rotation transform during training",
    )
    parser.add_argument(
        "--lr_scheduler",
        type=str,
...
def main(args):
    ...
    # Optimizer creation
    params_to_optimize = (
        itertools.chain(unet.parameters(), controlnet.parameters()) if args.unfreeze_unet else controlnet.parameters()
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                unet_context = (
                    torch.no_grad() if not args.unfreeze_unet else contextlib.nullcontext()
                )
                with unet_context:
                    model_pred = unet(
                        noisy_latents,
                ...
                if accelerator.sync_gradients:
                    params_to_clip = (itertools.chain(unet.parameters(), controlnet.parameters())
                        if args.unfreeze_unet
