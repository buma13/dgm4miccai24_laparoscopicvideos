...
### ADAPTED VERSION OF THE ORIGINAL CholecT50 DATALOADER from https://github.com/CAMMA-public/cholect50
...
class CholecT50():
    def __init__(self,
                 ...
                 test_fold=1,
                 augmentation_list=['original', 'vflip',
                                    'hflip', 'contrast', 'rot90'],
                 normalize=True):
        """ Args
        ...
        self.list_dataset_variant = {
            "cholect50_full": "entire CholecT50 dataset",
            "cholect50_test": "CholecT50 subset for testing",
        ...
        assert dataset_variant in self.list_dataset_variant.keys(), print(
            dataset_variant, "is not a valid dataset variant")
        self.video_split = self.split_selector(case=dataset_variant)
        train_videos = sum([v for k, v in self.video_split.items() if k != test_fold], [
        ]) if 'crossval' in dataset_variant else self.video_split['train']
        test_videos = sum([v for k, v in self.video_split.items() if k == test_fold], [
        ]) if 'crossval' in dataset_variant else self.video_split['test']
        if 'crossval' in dataset_variant:
            val_videos = train_videos[-5:]
            train_videos = train_videos[:-5]
        else:
            val_videos = self.video_split['val']
        self.train_records = ['VID{}'.format(
            str(v).zfill(2)) for v in train_videos]
        self.val_records = ['VID{}'.format(
            str(v).zfill(2)) for v in val_videos]
        self.test_records = ['VID{}'.format(
            str(v).zfill(2)) for v in test_videos]
        self.augmentations = {
        ...
        if dataset_variant != "cholect50_full":
    ...
    def split_selector(self, case='cholect50'):
        switcher = {
            'cholect50_full': {
                'train': [1, 15, 26, 40, 52, 65, 79, 2, 18, 27, 43, 56, 66, 92, 4, 22, 31, 47, 57, 68, 96, 5, 23, 35, 48, 60, 70, 103, 13, 25, 36, 49,
                          62, 75, 110, 6, 51, 10, 73, 14, 74, 32, 80, 42, 111, 8, 12, 29, 50, 78],
                'val': [],
                'test': []
            },
            'cholect50_test': {
            ...
            'cholect50-challenge': {
                'train': [1, 15, 26, 40, 52, 79, 2, 27, 43, 56, 66, 4, 22, 31, 47, 57, 68, 23, 35, 48, 60, 70, 13, 25, 49, 62, 75, 8, 12, 29, 50, 78,
    ...
    def transform(self):
        ...
        op_train = [transforms.Resize((256, 448))] + self.augmentation_list + [
            transforms.Resize((256, 448)), transforms.ToTensor()]
        if self.normalize:
    ...
    def build_train_dataset(self, transform):
        ...
        for video in self.train_records:
            dataset = T50(img_dir=os.path.join(self.dataset_dir, 'videos', video),
                          label_file=os.path.join(
                              self.dataset_dir, 'labels', '{}.json'.format(video)),
                          transform=transform,
        ...
        self.train_set_list = iterable_dataset
    ...
    def build_val_dataset(self, transform):
        ...
        self.val_set_list = iterable_dataset
    ...
    def build_test_dataset(self, transform):
        ...
        self.test_set_list = iterable_dataset
...
class T50(Dataset):
    def __init__(self, img_dir, label_file, transform=None, target_transform=None):
        ...
        self.label_categories = label_data["categories"]
    ...
    def __getitem__(self, index):
        ...
        base_name = "{}.png".format(str(self.frames[index]).zfill(6))
        img_path = os.path.join(self.img_dir, base_name)
        rel_img_path = img_path.split("/")[-2] + "/" + img_path.split("/")[-1]
        ...
        phase_id = str(labels[4].nonzero()[0][0].item())
        phase_text = self.label_categories['phase'][phase_id]
        # try:
        triplet_ids = labels[0].nonzero().squeeze().tolist()
        if isinstance(triplet_ids, int):
            annotation_text = self.label_categories['triplet'][str(
                triplet_ids)] + " in " + phase_text
        elif len(triplet_ids) > 1:
            triplet_texts = [self.label_categories['triplet']
                             [str(triplet_id)] for triplet_id in triplet_ids]
            annotation_text = " and ".join(triplet_texts) + \
                " in " + phase_text
        else:
            tools = [self.label_categories["instrument]"]
                     [idx] if x == 1 else None for idx, x in enumerate(labels[1])]
            tools = [x.lower() for x in tools if x is not None]
            annotation_text = " and ".join(tools) + \
                " in " + phase_text if len(tools) > 0 else phase_text

        return image, labels, rel_img_path, annotation_text
